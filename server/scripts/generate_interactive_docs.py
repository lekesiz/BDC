...
def load_openapi_spec(spec_file):
    ...
    try:
        ...
        logger.info(f"Error: OpenAPI specification file '{spec_file}' not found.")
        sys.exit(1)
...
def serve_documentation(output_dir, port=8080):
    ...
    try:
        ...
        if e.errno == 48:  # Port already in use
            logger.info(f"Error: Port {port} is already in use. Try a different port with --port option.")
        else:
...
def main():
    ...
    if not os.path.exists(args.spec):
        logger.info(f"Error: OpenAPI specification file '{args.spec}' not found.")
        logger.info("Make sure you have the openapi_specification.yaml file in the current directory.")
        sys.exit(1)
    ...
    logger.info(f"✅ Interactive API documentation generated successfully!")
    logger.info(f"📁 Output directory: {args.output}")
    logger.info(f"📄 Main file: {html_file}")
    ...
    if not args.no_serve:
        ...
        logger.info(f"Or run: python -m http.server {args.port} --directory {args.output}")
