#!/usr/bin/env python3
"""Check if coverage has increased between two coverage.xml files."""
import sys
import xml.etree.ElementTree as ET
from typing import Tuple

from app.utils.logging import logger


def parse_coverage(xml_file: str) -> Tuple[float, dict]:
    """Parse coverage XML file and return coverage percentage and details."""
    try:
        tree = ET.parse(xml_file)
        root = tree.getroot()
        
        # Get overall line rate
        line_rate = float(root.get('line-rate', 0)) * 100
        
        # Get package-level coverage
        packages = {}
        for package in root.findall('.//package'):
            package_name = package.get('name')
            package_line_rate = float(package.get('line-rate', 0)) * 100
            packages[package_name] = package_line_rate
        
        return line_rate, packages
    except Exception as e:
        logger.info(f"Error parsing {xml_file}: {e}")
        return 0.0, {}


def main():
    """Main function to compare coverage."""
    if len(sys.argv) != 3:
        logger.info("Usage: python check_coverage_increase.py <base_coverage.xml> <current_coverage.xml>")
        sys.exit(1)
    
    base_file = sys.argv[1]
    current_file = sys.argv[2]
    
    base_coverage, base_packages = parse_coverage(base_file)
    current_coverage, current_packages = parse_coverage(current_file)
    
    logger.info(f"\n{'='*60}")
    logger.info("COVERAGE COMPARISON")
    logger.info(f"{'='*60}")
    logger.info(f"Base coverage:    {base_coverage:.2f}%")
    logger.info(f"Current coverage: {current_coverage:.2f}%")
    logger.info(f"Difference:       {current_coverage - base_coverage:+.2f}%")
    
    # Check if coverage increased
    if current_coverage < base_coverage:
        logger.info("\n❌ COVERAGE DECREASED!")
        logger.info("This PR must increase or maintain test coverage.")
        
        # Show which packages decreased
        logger.info(f"\n{'='*60}")
        logger.info("PACKAGES WITH DECREASED COVERAGE:")
        logger.info(f"{'='*60}")
        
        for package, current_rate in current_packages.items():
            base_rate = base_packages.get(package, 0)
            if current_rate < base_rate:
                logger.info(f"{package}: {base_rate:.2f}% → {current_rate:.2f}% ({current_rate - base_rate:+.2f}%)")
        
        sys.exit(1)
    
    elif current_coverage == base_coverage:
        logger.info("\n⚠️  Coverage remained the same.")
        logger.info("Consider adding more tests to increase coverage.")
    
    else:
        logger.info("\n✅ Coverage increased!")
        
        # Show which packages improved
        logger.info(f"\n{'='*60}")
        logger.info("PACKAGES WITH INCREASED COVERAGE:")
        logger.info(f"{'='*60}")
        
        for package, current_rate in current_packages.items():
            base_rate = base_packages.get(package, 0)
            if current_rate > base_rate:
                logger.info(f"{package}: {base_rate:.2f}% → {current_rate:.2f}% ({current_rate - base_rate:+.2f}%)")
    
    # Show new packages
    new_packages = set(current_packages.keys()) - set(base_packages.keys())
    if new_packages:
        logger.info(f"\n{'='*60}")
        logger.info("NEW PACKAGES:")
        logger.info(f"{'='*60}")
        for package in new_packages:
            logger.info(f"{package}: {current_packages[package]:.2f}%")
    
    # Show removed packages
    removed_packages = set(base_packages.keys()) - set(current_packages.keys())
    if removed_packages:
        logger.info(f"\n{'='*60}")
        logger.info("REMOVED PACKAGES:")
        logger.info(f"{'='*60}")
        for package in removed_packages:
            logger.info(f"{package}: {base_packages[package]:.2f}%")
    
    logger.info(f"\n{'='*60}\n")


if __name__ == "__main__":
    main()