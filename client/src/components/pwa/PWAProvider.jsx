 ...
 */
export function PWAStatus({ className = '' }) {const { t } = useTranslation();
  const { isOnline, canInstall, isInstalled, hasUpdate, isLoading } = usePWAContext();
  return (
    <div className={`space-y-2 ${className}`}>
      <div className="text-sm font-medium">{t("components.pwa_status")}</div>
      <div className="grid grid-cols-2 gap-2 text-xs">
        <div className={`p-2 rounded ${isOnline ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>{t("components.connection")}
        ...
        {canInstall &&
        <div className="p-2 rounded bg-blue-100 text-blue-800">{t("components.install_available")}

        </div>
        }
        {hasUpdate &&
        <div className="p-2 rounded bg-orange-100 text-orange-800">{t("components.update_available")}

        </div>
        }
        {isLoading &&
        <div className="p-2 rounded bg-yellow-100 text-yellow-800">{t("components.status_loading")}
 ...
 */
export function PWAControls({ className = '' }) {const { t } = useTranslation();
  const { canInstall, hasUpdate, isLoading, install, update } = usePWAContext();
  return (
    <div className={`space-y-2 ${className}`}>
      <div className="text-sm font-medium">{t("components.pwa_controls")}</div>
      <div className="flex flex-wrap gap-2">
        ...
        <button
          ...
          className="px-3 py-1 text-xs bg-gray-600 text-white rounded hover:bg-gray-700">{t("components.reload")}
